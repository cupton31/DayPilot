...
namespace DayPilot.Web.Ui
{
    ...
    public class DayPilotScheduler : DataBoundControl, IPostBackEventHandler
    {
        ...
        private enum _DurationBarColor { Crimson, MediumVioletRed, OrangeRed, Indigo, DarkGreen, Blue, DeepSkyBlue, SaddleBrown, Black }
        private enum _BackgroundColor { LightSalmon, LightPink, Khaki, Thistle, PaleGreen, PowderBlue, LightCyan, Wheat, Gainsboro }
        ...
        public List<string> RowHeaderTitles
        {
            get { 
        ...
        private void RenderMainTable(HtmlTextWriter output)
        {
            ...
            output.AddStyleAttribute("margin-left", "250px");
            ...
            output.AddStyleAttribute("overflow-x", "scroll");
            output.AddStyleAttribute("width", "85%");
        ...
        private void RenderEvent(Day d, Event p, HtmlTextWriter output)
        {
            ...
            ea.InnerHTML = "<b>" + p.Text + "</b>";
            if (ShowHours) { ea.InnerHTML = "<b>" + p.Text + "</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; " + p.Start.ToString("HH:mm") + "-" + p.End.ToString("HH:mm") + ""; }
            ea.ToolTip = p.Text;
            ea.EventClickEnabled = EventClickHandling != EventClickHandlingEnum.Disabled;
            if (!CssOnly)
            {
                ea.DurationBarColor = Enum.GetName(typeof(_DurationBarColor), Int32.Parse(d.Value) % 8);
                ea.BackgroundColor = Enum.GetName(typeof(_BackgroundColor), Int32.Parse(d.Value) % 8);//ColorTranslator.ToHtml(EventBackColor);
            }
            ...
            int durationBarLeft = left;
            int durationBarWidth = width;

            if (EventWidthShouldEqualCellWidth) {
                if ((int)Math.Floor((p.BoxStart - d.Start).TotalMinutes * CellWidth / CellDuration) < 0) return;
                left = (int)Math.Floor((p.BoxStart.Date - d.Start).TotalMinutes * CellWidth / CellDuration);
                width = (int) CellWidth - 2;
                durationBarWidth = width;
                realWidth = width;
            } 
            ...
            if (!CssOnly)
            {
                ...
                {
                    output.Write("<div unselectable='on' style='width:" + realWidth + "px; margin-left: " + startDelta + "px; height:2px; background-color:" + Enum.GetName(typeof(_DurationBarColor), Int32.Parse(d.Value) % 8) + "; font-size:1px; position:relative;' ></div>");
                    output.Write("<div unselectable='on' style='width:" + durationBarWidth + "px; height:1px; background-color:" + ColorTranslator.ToHtml(EventBorderColor) + "; font-size:1px; position:relative;' ></div>");
                }
            ...
            else
            {
                ...
                if (DurationBarVisible)
                {
                    ...
                    double barLeft = 100.0 * startDelta / durationBarWidth;
                    double barWidth = 100.0 * realWidth / durationBarWidth;
        ...
        private void RenderRowHeader(HtmlTextWriter output, Day d)
        {
            ...
            output.AddStyleAttribute("top", "auto");
            output.AddStyleAttribute("position", "absolute");
            output.AddStyleAttribute("top", "auto");
            output.AddStyleAttribute("border-top-width", "1px");
            ...
            if (!CssOnly)
            {
                ...
                output.AddStyleAttribute("background-color", Enum.GetName(typeof(_BackgroundColor), Int32.Parse(d.Value) % 8));
                output.AddStyleAttribute("font-family", HeaderFontFamily);
                output.AddStyleAttribute("font-size", HeaderFontSize);
                output.AddStyleAttribute("color", Enum.GetName(typeof(_DurationBarColor), Int32.Parse(d.Value) % 8));
                output.AddStyleAttribute("cursor", "default");
            }
            ...
            if (RowHeaderColumns.Count > 0)
            {
                if (!CssOnly)
                {
                    ...
                    sb.Append("<tr>");
                    sb.Append("<td style='width:" + RowHeaderColumns[0] + "px; padding: 0px; font-weight:bold; white-space: nowrap; '>");
        ...
        internal void RenderHeaderCols(HtmlTextWriter output)
        {
            ...
            for (int i = 0; i < CellCount; i++)
            {
                ...
                else if (CellDuration < 1440 * 28) // smaller than smallest month, use days
                {
                    ...
                    text = tooltip.Split(',')[0]+"   ["+tooltip.Split(',')[1].Remove(0,1)+"]";
                }
        ...
        [Category("Appearance")]
        [DefaultValue(true)]
        public bool EventWidthShouldEqualCellWidth
        {
            get
            {
                if (ViewState["EventWidthShouldEqualCellWidth"] == null)
                    return true;

                return (bool)ViewState["EventWidthShouldEqualCellWidth"];
            }
            set
            {
                ViewState["EventWidthShouldEqualCellWidth"] = value;
            }
        }
