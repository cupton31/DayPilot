...
//                output.Write(divPosition.EndTag());
        ...
        private void RenderEvent(HtmlTextWriter output, Event e, Day d)
        {
            ...
            if (!CssOnly)
            {
                ea.DurationBarColor = Enum.GetName(typeof(_DurationBarColor), Int32.Parse(d.Value) % 8);//ColorTranslator.ToHtml(DurationBarColor);
            ...
            if (CssOnly)
            {
                ...
                {
                    output.AddStyleAttribute("background", Enum.GetName(typeof(_BackgroundColor), Int32.Parse(d.Value) % 8));
                }
                output.RenderBeginTag("div");
                ...
                {
                    output.AddStyleAttribute("background-color", Enum.GetName(typeof(_DurationBarColor), Int32.Parse(d.Value) % 8));
                }
                output.RenderBeginTag("div");
            ...
            else
            {
                ...
                if (ea.EventClickEnabled && EventClickHandling != EventClickHandlingEnum.Disabled)
                {
                    if (!CssOnly)
                    {
                        output.AddAttribute("onmouseover", "this.style.backgroundColor='" + Enum.GetName(typeof(_BackgroundColor), Int32.Parse(d.Value) % 8) + "';event.cancelBubble=true;");
                        output.AddAttribute("onmouseout", "this.style.backgroundColor='" + Enum.GetName(typeof(_BackgroundColor), Int32.Parse(d.Value) % 8) + "';event.cancelBubble=true;");
                    }
                }
